...
public class ZillaViewModel(private val zillaRepository: ZillaRepository) : ViewModel() {
    ...
    private fun verifyOrderId(orderId: String) {
        ...
        viewModelScope.launch {
            ...
            zillaRepository.validateOrderId(orderId).collect {
    ...
    private fun createWithPublicKey(params: ZillaParams) {
        ...
        viewModelScope.launch {
            _loadingStatus.emit(true)
            zillaRepository.createWithPublicKey(CreateWithPublicKeyRequest.fromZillaParams(params))
                .collect {
                    _loadingStatus.emit(false)
                    when (it) {
                        is Result.Success -> {
